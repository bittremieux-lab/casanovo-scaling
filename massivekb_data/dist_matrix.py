...
def square_dist_matrix(sequences, cache_dir=None, n_threads=cpu_count()):
    ...
    with Pool(n_threads, initializer=_square_init_pool, initargs=(sequences,)) as pool:
        for i, row in tqdm(pool.imap_unordered(_square_compute_row, range(n - 1)), total=n - 1):
...
def rectangle_distance_matrix(seqs1, seqs2, n_threads=cpu_count()):
    ...
    with Pool(n_threads, initializer=_rect_init_pool, initargs=(seqs1, seqs2)) as pool:
        for i, row in tqdm(pool.imap_unordered(_rect_compute_row, range(n1)), total=n1):
            dist_matrix[i, :] = row
